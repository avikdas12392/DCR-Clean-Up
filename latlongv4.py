...
# India guardrails (still helpful even with correct OLat/OLong)
...
# Default Output1 column schema (Output2 is built dynamically: input_cols + these + FuzzyScore)
OUTPUT1_COLS_FIXED = [
...
def print_call_purpose(row_idx: int, olat: Any, olon: Any, pin: str, assoc: str, using_cache: bool = False):
    ...
    print(f"[{now_iso()}] Row {row_idx}: Search '{assoc or 'hospital'}' within {RADIUS_METERS}m via {src}. Inputs: OLat={olat}, OLong={olon}, pin={pin}. Limit={NUM_RESULTS}")
...
def in_india_or_has_indian_pin(addr: str, lat: Any, lon: Any) -> bool:
    ...
    try:
        ...
        pass
    # Or address clearly mentions India or has a 6-digit PIN
...
def ensure_excel_with_sheet(path: Path, columns: List[str], sheet_name: str = "Sheet1") -> None:
    if path.exists():
        # ensure header row exists if file is empty
...
def call_serper_or_cache(cache_db: sqlite3.Connection, assoc: str, olat: Any, olon: Any, pin: str) -> Optional[Dict[str, Any]]:
    ...
    base = (assoc or "hospital").strip()
    q = f"{base} near {olat},{olon} {pin} India".strip()
...
def main():
    ...
    output1_cols = OUTPUT1_COLS_FIXED[:]  # copy
    ...
    for idx in range(last_done + 1, n):
        ...
        try:
            ...
            # Quick sanity check of coords (do not auto-swap, per your instruction)
            ...
            for p in places[:NUM_RESULTS]:
                ...
                # Strict India filter (safety net)
                ...
                # Output 1: only if globally new
                if is_new_globally_and_mark(dedupe_db, key):
                    out1_rows.append(r1)

                # Output 2: fuzzy match against Tagged Address
                score = fuzzy_score((addr or "").lower(), (tagged_addr or "").lower())
                if score >= FUZZY_THRESHOLD:
                    left = {c: row.get(c, "") for c in input_headers}  # all input cols, includes OLat and OLong
