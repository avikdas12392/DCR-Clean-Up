...
- Output 1: allowed (whitelist & reviews>=75) + Eliminated (others)
- Output 2: writes ONE row per input row (always). If a match passes gates, attach best candidate; else leave Output-1 fields blank.
"""
...
try:
    ...
    def fuzzy_partial(a: str, b: str) -> float:
        return float(fuzz_partial_ratio(a or "", b or ""))
except Exception:
    from difflib import SequenceMatcher
    def _sm(a, b): return 100.0 * SequenceMatcher(None, (a or ""), (b or "")).ratio()
    fuzzy_score = _sm
    fuzzy_partial = _sm
...
NUM_RESULTS = 20
RADIUS_METERS = 200              # keep as in your file
TIMEOUT = 30
BASE_SLEEP = 0.8
BACKOFF_FACTOR = 1.6
MAX_RETRIES = 4
...
# Match thresholds / shortlist sizes
SHORTLIST_NEAREST = 8            # ↑ shortlist a bit to improve recall
OUT2_MIN_SCORE = 75.0            # need this score to attach a match to Output 2
...
# ✅ Whitelist (unchanged as provided)
...
# ---------- IMPROVED matching for Output-2 ----------
def build_match_score(
    ...
    """Blended score: distance + pin + brand + locality + name similarity + category + review confidence."""
    ...
    # Name similarity (Associate Hospital vs candidate title)
    assoc_norm = norm_text(assoc)
    cand_name_norm = norm_text(cand_name)
    name_sim = 0.6 * fuzzy_score(assoc_norm, cand_name_norm) + 0.4 * fuzzy_partial(assoc_norm, cand_name_norm)
    ...
    try:
        ...
        rv = 0
    # review confidence: 0–100 ~ min(100, 10 * sqrt(reviews))
    rev_conf = min(100.0, 10.0 * math.sqrt(max(0, rv)))

    # Gate-weighted score
    score = (
        0.40 * ds +                                # proximity
        0.22 * (100.0 if pin_ok else 0.0) +        # pin
        0.12 * min(100.0, bh * 50.0) +             # brand tokens
        0.12 * loc +                                # locality overlap
        0.10 * name_sim +                           # name similarity
        0.02 * (100.0 if cat_ok else 0.0) +        # category
        0.02 * rev_conf                             # review confidence
    )
    # Soft penalize far without PIN
    if (not pin_ok) and d_km > 0.3:
        score *= 0.85
    info = {
        "d_km": round(d_km, 4),
        "pin_ok": pin_ok,
        "brand_hits": bh,
        "loc_score": round(loc, 1),
...
def hard_override(
    ...
    if brand_hits(assoc, cand_name, cand_addr) > 0:
        if haversine_km(olat, olong, cand_lat, cand_lon) <= 0.5 and pin_equal(pin, cand_pin, cand_addr):
            return True
    return False
    ...
    for i in range(last_done + 1, n):
        ...
        try:
            ...
            if olat is None or olong is None:
                ...
                # Still write a blank Output-2 row to preserve row count
                left = {c: row.get(c, "") for c in headers}
                empty_match = {k:"" for k in OUTPUT1_COLS}
                append_rows_excel(OUTPUT2_FILE, [{**left, **empty_match, "FuzzyScore": 0}], out2_cols, "Sheet1")
                continue
            ...
            # ---- Split Output1: allowed (whitelist & reviews>=75) vs Eliminated (others) ----
            def reviews_int(x):
                try: return int(str(x or "0").replace(",", ""))
                except: return 0

            allowed_for_out1, eliminated_for_out1 = [], []
            for rec in collected:
                catnorm = (rec.get("Category","") or "").lower().strip()
                rv = reviews_int(rec.get("Reviews"))
                if (catnorm in WHITELIST_CATEGORIES) and (rv >= 75):
                    allowed_for_out1.append(rec)
                else:
                    eliminated_for_out1.append(rec)

            # Output1 with global dedupe for allowed; eliminated is written as-is
            out1_to_write = []
            for r2 in allowed_for_out1:
            ...
            if eliminated_for_out1:
                append_rows_excel(OUTPUT1_FILE, eliminated_for_out1, OUTPUT1_COLS, "Eliminated")

            # ---- Output-2 selection (STRICT gate: whitelist + reviews>=75) ----
            gated = [r for r in allowed_for_out1]  # already satisfies category & reviews>=75

            # shortlist by nearest
            def dist_of(r):
                return haversine_km(olat, olong, r["Lat"] or 0, r["Long"] or 0)
            gated.sort(key=dist_of)
            shortlist = gated[:SHORTLIST_NEAREST]
            ...
            # ---- Output-2 write: ALWAYS one row per input row ----
            left = {c: row.get(c, "") for c in headers}
            if best and (best_score >= OUT2_MIN_SCORE):
                out_row = {**left, **best, "FuzzyScore": round(best_score, 2)}
            else:
                # no acceptable match → keep Output1 fields blank but preserve row count
                empty_match = {k:"" for k in OUTPUT1_COLS}
                out_row = {**left, **empty_match, "FuzzyScore": 0}
            append_rows_excel(OUTPUT2_FILE, [out_row], out2_cols, "Sheet1")

            print(f"[{now_iso()}] ✅ Row {i}: "
                  f"{len(out1_to_write)} allowed→Output1, "
                  f"{len(eliminated_for_out1)} eliminated, "
                  f"Output2 score={'{:.2f}'.format(best_score) if best_score>=0 else 'NA'} {best_info if best else ''}")
            ...
            progress["last_processed_input_row"] = i
            save_progress(progress)
            # still preserve Output2 row count
            left = {c: row.get(c, "") for c in headers}
            empty_match = {k:"" for k in OUTPUT1_COLS}
            append_rows_excel(OUTPUT2_FILE, [{**left, **empty_match, "FuzzyScore": 0}], out2_cols, "Sheet1")
            continue
